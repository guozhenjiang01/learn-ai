...
public class MediaScriptService {
    ...
    public String generateCommutePack(List<String> topics, String extra, String weather) {
        ...
        for (String t : topics) {
            switch (t) {
                case "周五回太原" -> topicDesc.append("""
                    ○ 周五晚·回太原：亦庄收工→北京丰台站→G551高铁→太原站→孩子出站口→到家热饭。情绪：疲惫中的期待
                    """);
                case "周末日常" -> topicDesc.append("""
                    ○ 周末·太原日常：带娃/菜市场/公园/老婆（北大博士）/收拾行李准备返京。情绪：温馨日常+即将分别的小情绪
                    """);
                case "周一返京" -> topicDesc.append("""
                    ○ 周一早·返京通勤：早起→太原站→高铁→亦庄京东→工位开机→周一恐惧。情绪：牛马上钟的无奈
                    """);
            }
        }
        ...
        String prompt = """
            ...
            %s%s
            请将以下%d个话题融合成一条25-40秒的抖音脚本（只生成一条！）：
            %s
            
            ═══ 融合规则 ═══
            1. 把上述话题串成一个完整故事线，不要分成独立段落
            2. 用时间线或情绪线自然过渡（如：周五收工→路上→到家→周末→周一返京）
            3. 3个话题全选时按时间顺序串联；只选1-2个时深耕该场景
            4. 前后呼应，形成一条完整的情绪弧线
